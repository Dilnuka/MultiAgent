...
# Lazy PDF support via reportlab (imported when needed)
REPORTLAB_AVAILABLE = None  # type: ignore[assignment]
...
def _ensure_reportlab():
    ...
    if REPORTLAB_AVAILABLE is True:
        return True
    try:
        # Import here to reflect newly installed deps without restarting Python
...
def _md_to_pdf_bytes(markdown_text: str) -> bytes:
    ...
    # Imports after ensuring availability
...
def _render_report_view(text: str, topic_for_filename: str, mtime: float | None = None):
    ...
    st.markdown("### 📋 Report (original)")
    st.markdown(text)
    if _ensure_reportlab():
        try:
